...
| measurement | result |
| --- | --- |
| CPU/Metal divergence, greedy | none in six generated tokens on "The capital of France is" (both: " Paris. The capital of Italy"); the CPU took 72 s to the first token against 0.42 s |
| readback share of a decode step | 0.21% at f16, 0.10% at int8 |
| decode | f16 18.04 tokens/s (p50 53.8 ms, device 91.9%, submit 6.6%) in the morning; **27.83 tokens/s (p50 34.6 ms, device 95.1%)** after accel bounded its attention loops by the length and split its matvecs sixteen ways, both found by attributing this step; int8 20.59 tokens/s before those two changes |
| context sensitivity | 256 positions of capacity: 21.6 tokens/s against 18.0 at 4096, before the attention change; the difference was the empty blocks the kernel walked |
| quantization error on real blocks | not measured: `bench` compares no host reference, and accel's `Int4ErrorBound` was re-derived over the stored scale on 2026-09-02 (accel specs/048) |
