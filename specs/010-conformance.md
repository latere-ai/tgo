...
| C22 | a **ragged step over an f16 cache** | 046, 010 | [#23](https://github.com/golang-design/accel/issues/23) | **open** | an f32 cache on the batched path, so [C5](#2-the-register)'s halving is given back by the operator that makes batching possible. Per-sequence traffic $A$ doubles, and [008 §1](008-scheduler.md) makes the batch size worth reaching and the throughput ceiling both proportional to $1/A$ |
| C23 | a **ragged step that tolerates a query row belonging to no sequence** | 046, 010 | [#24](https://github.com/golang-design/accel/issues/24) | **open** | a host-side invariant, `sum(QueryExtents) == q.shape[0]`, maintained by tgo and untestable by accel — the extents are device data, so the sum is not known at record time. A batched step therefore pads by inflating a real sequence's extent rather than by leaving rows unclaimed, and a bucketed batch cannot pad `q` freely |
